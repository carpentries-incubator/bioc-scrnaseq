...
For example, we compute QC metrics below with the same `perCellQCMetrics()`
...
that is used in many functions, e.g., `bluster::makeSNNGraph()`,
...
analyses, e.g., in `denoisePCA()`, `fastMNN()`,
`scDblFinder::computeDoubletDensity()`. (Briefly, the
