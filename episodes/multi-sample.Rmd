...
We "correct" the effect of samples with the `correctExperiments` function
...
`edgeR` package within the `normLibSizes` function (formerly `calcNormFactors`).
...
We don't normalize the abundance data with the `normLibSizes` function, as this would
...
To do so, we first normalize the data with `normLibSizes` and then we fit and
...
-   Computational removal of batch-to-batch variation with the `correctExperiments`
...
-   The `aggregateAcrossCells` function from the `r Biocpkg("scuttle")` package
